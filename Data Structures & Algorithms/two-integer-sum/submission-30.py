class Solution:
    def twoSum(self, nums: List[int], target: int) -> List[int]:
        ...
        """Approcah 3+ : Modified : Combine Step1 and Step 2 in one scan; Best Solution """
        # continuously build and use in one scan
        dct = {}
        for i in range(len(nums)):
            pair = target - nums[i]

            if pair in dct :
                return [dct.get(pair),i]
            dct[nums[i]] = i 
